...
class Ship extends Projectile {
    ...
    get blinkOn() {
        if (this.blinking)  { return this.blinkingTimer % (SHIP_BLINK_RATE * FPS * 2) < SHIP_BLINK_RATE * FPS }
        else                { return true}
    }
    ...
    accelerate() {
        if (this.thrust) {
            if (this.v.speed < SHIP_MAX_SPEED) {
                this.v.dx += SHIP_THRUST_AMOUNT * Math.sin(this.a + rad(90)) / FPS
                this.v.dy += SHIP_THRUST_AMOUNT * Math.cos(this.a + rad(90)) / FPS
